...
graduates.forEach((graduate) => {
 return universities.push(graduate.university);
})
...
graduates.forEach((graduate) => {
 if(graduate.university.includes('Uni')){
   return unisWithUni.push(graduate.university);
 }
})
...
zooAnimals.filter((currentValue)=> {
if(currentValue.population < 5) {
  return lowPopulationAnimals.push(currentValue); 
}
})
...
*/
const populationTotal = zooAnimals.reduce((accum, currentValue) => {
